...
class Cut(object):
    ...
    def _ordinal_array(self):
        ...
        ordinal_array = np.array([[3.0, 1.0], [2.0, 0.0]])
    ...
    def nestedArray2nestedMap(self, map_cut):
        ...
        array_fill = map_cut[array_fill.reshape(1, -1).astype(int)].reshape(
            2 ** (circle_num + 1), 2 ** (circle_num + 1)
        )
...
class Block2Full(Cut):
    ...
    def _full(self, map_block):
        ...
        map_NEST[self.block_n * self.nside**2 : (self.block_n + 1) * self.nside**2] = (
            map_block_array
        )
...
def sphere2piecePlane_squa_pad(sphere_map, nside=512, padding_size=128):
    ...
    piece_map[3 * nside : 4 * nside, 0:nside] = blocks[0]
    #
    piece_map[2 * nside : 3 * nside, nside - 32 * mult : nside] = np.rot90(
        blocks[0][0 : 32 * mult, :], k=3, axes=(0, 1)
    )  # np.rot90(blocks[0], k=3, axes=(0, 1))
    piece_map[3 * nside - 32 * mult : 3 * nside, 0:nside] = np.rot90(
        blocks[1][:, 0 : 32 * mult], k=1, axes=(0, 1)
    )  # np.transpose(blocks[1][:, 0:32*3])  #np.rot90(blocks[1][:, 0:32], k=1, axes=(0, 1))
    piece_map[1 * nside : 2 * nside, (2 * nside) - 32 * mult : 2 * nside] = np.rot90(
        blocks[1][0 : 32 * mult, :], k=3, axes=(0, 1)
    )
    piece_map[2 * nside - 32 * mult : 2 * nside, nside : nside * 2] = np.rot90(
        blocks[2][:, 0 : 32 * mult], k=1, axes=(0, 1)
    )
    piece_map[0 * nside : 1 * nside, (3 * nside) - 32 * mult : 3 * nside] = np.rot90(
        blocks[2][0 : 32 * mult, :], k=3, axes=(0, 1)
    )
    piece_map[1 * nside - 32 * mult : 1 * nside, 2 * nside : nside * 3] = np.rot90(
        blocks[3][:, 0 : 32 * mult], k=1, axes=(0, 1)
    )
    ...
    piece_map[
        pad + 5 * nside : pad + 5 * nside + 32 * mult, pad + 1 * nside : pad + nside * 2
    ] = np.rot90(blocks[11][:, nside - 32 * mult : nside], k=1, axes=(0, 1))
    piece_map[
        pad + 1 * nside : pad + 2 * nside, pad + 5 * nside : pad + 5 * nside + 32 * mult
    ] = np.rot90(blocks[8][nside - 32 * mult : nside, :], k=3, axes=(0, 1))
    piece_map[pad - 32 * mult : pad, pad + 3 * nside : pad + nside * 4] = np.rot90(
        blocks[0][:, 0 : 32 * mult], k=1, axes=(0, 1)
    )
    piece_map[pad + 3 * nside : pad + 4 * nside, pad - 32 * mult : pad] = np.rot90(
        blocks[3][0 : 32 * mult, :], k=3, axes=(0, 1)
    )

    piece_map[pad + 4 * nside : pad + 5 * nside, pad - 32 * mult : pad] = blocks[3][
        :, nside - 32 * mult : nside
    ]
    piece_map[
        pad + 5 * nside : pad + 5 * nside + 32 * mult, pad + 0 * nside : pad + 1 * nside
    ] = blocks[11][0 : 32 * mult, :]
    piece_map[pad - 32 * mult : pad, pad + 4 * nside : pad + 5 * nside] = blocks[0][
        nside - 32 * mult : nside, :
    ]
    piece_map[
        pad + 0 * nside : pad + 1 * nside, pad + 5 * nside : pad + 5 * nside + 32 * mult
    ] = blocks[8][:, 0 : 32 * mult]
...
def sphere2piecePlane_squa_pad_mult(sphere_map, nside=512, padding_size=128):
    ...
    if multi_map:
        piece_map = np.zeros(
            (multi_map_n, nside * 5 + 2 * padding_size, nside * 5 + 2 * padding_size)
        )
        for i in range(multi_map_n):
            sphere_map_i = sphere_map[i]
            piece_map[i, :] = sphere2piecePlane_squa_pad(
                sphere_map_i, nside, padding_size
            )
...
def piecePlane_squa2sphere_mult(piece_maps, nside=512):
    ...
    if multi_map:
        ...
        for i in range(multi_map_n):
            ...
            for ii in range(12):
                full_map = Block2Full(
                    blocks["block_%s" % ii], ii, base_map=base_map_i
                ).full()
                base_map_i = full_map
...
def piecePlane_squa2sphere_pad(piece_map, nside=512, padding_size=128):
    ...
    this is only for the case of subblocks_nums=1
    """
    blocks = piecePlane2blocks_squa_pad(
        piece_map, nside=nside, padding_size=padding_size
    )
    base_map = np.zeros(12 * nside**2)
    for i in range(12):
        full_map = Block2Full(blocks["block_%s" % i], i, base_map=base_map).full()
...
def piecePlane_squa2sphere_pad_mult(piece_maps, nside=512, padding_size=128):
    ...
    if multi_map:
        ...
        for i in range(multi_map_n):
            base_map_i = np.zeros(12 * nside**2)
            blocks = piecePlane2blocks_squa_pad(
                piece_maps[i, :], nside=nside, padding_size=padding_size
            )
            for ii in range(12):
                full_map = Block2Full(
                    blocks["block_%s" % ii], ii, base_map=base_map_i
        ...
        return piecePlane_squa2sphere_pad(
            piece_maps, nside=nside, padding_size=padding_size
        )
...
def blockPlane2sphere_mult(block_maps, nside=256, block_n="block_0"):
    ...
    if multi_map:
        base_map = np.zeros((multi_map_n, 12 * nside**2))
        for i in range(multi_map_n):
            full_map_i = Block2Full(
                block_maps[i].swapaxes(-1, -2), block_n=int(block_n[-1]), base_map=None
            ).full()  # Note! .T
            base_map[i, :] = full_map_i
        return base_map
    else:
        return Block2Full(
            block_maps.swapaxes(-1, -2), block_n=int(block_n[-1]), base_map=None
        ).full()  # Note! .T
