...
train_config_random_one_05 = {
    "syn_spectralindex_random": (0.05, "one"),
    "syn_amplitude_random": (0, "one"),
    "dust_spectralindex_random": (0.05, "one"),
    "dust_amplitude_random": (0.0, "one"),
    "dust_temp_random": (0.05, "multi"),
    "ame_amplitude_random": (0, "one"),
    "Random_types_of_cosmological_parameters": "Uniform",
}
...
class Simulator_data(object):
    def __init__(
        ...
        config_random=train_config_random_one_05,
        out_unit="uK_CMB",
        save_dir="DATA/",
        using_beam=True,
        is_polarization_data=True,
        using_ilc_cmbmap=False,
    ):
        """
    ...
    def __get_noise(self):
        ...
        noise = data.noiser(
            Sens=self.noise_level, is_half_split_map=self.is_half_split_map
        )
        if self.is_polarization_data:
            noise = noise[:, 1:, :]
        return noise
    ...
    def _cread_file_name(self):
        ...
        for ty in ["training_set", "validation_set", "testing_set"]:
            ...
            dir_key["dir_noise_ILC_" + ty[0:3]] = (
                self.save_dir + "noise/{}_ILC/".format(ty)
            )
            dir_key["dir_noise_{}_reshape".format(ty[0:3])] = (
                self.save_dir + "noise/{}_reshape/".format(ty)
            )
            dir_key["dir_cmb_obs_" + ty[0:3]] = (
                self.save_dir + "observed/{}/cmb/".format(ty)
            )
            dir_key["dir_cmb_ilc_" + ty[0:3]] = (
                self.save_dir + "observed/{}/cmb_ilcnoise/".format(ty)
            )
            dir_key["dir_total_obs_" + ty[0:3]] = (
                self.save_dir + "observed/{}/total/".format(ty)
            )
            dir_key["dir_cmb_obs_flat_" + ty[0:3]] = (
                self.save_dir + "observed_flat_map/{}/cmb/".format(ty)
            )
            dir_key["dir_cmb_obs_flat_block_" + ty[0:3]] = (
                self.save_dir + "observed_flat_block_map/{}/cmb/".format(ty)
            )
            dir_key["dir_total_obs_flat_" + ty[0:3]] = (
                self.save_dir + "observed_flat_map/{}/total/".format(ty)
            )
            dir_key["dir_total_obs_flat_block_" + ty[0:3]] = (
                self.save_dir + "observed_flat_block_map/{}/total/".format(ty)
            )
            dir_key["dir_cmb_ilc_flat_{}".format(ty[0:3])] = (
                self.save_dir + "observed_flat_map/{}/cmb_ilcnoise/".format(ty)
            )
            dir_key["dir_cmb_ilc_flat_block_{}".format(ty[0:3])] = (
                self.save_dir + "observed_flat_block_map/{}/cmb_ilcnoise/".format(ty)
            )
            if not ty == "training_set":
                dir_key["dir_noise_ILC_{}_1".format(ty[0:3])] = (
                    self.save_dir + "noise/{}_ILC/half_1/".format(ty)
                )
                dir_key["dir_noise_ILC_{}_2".format(ty[0:3])] = (
                    self.save_dir + "noise/{}_ILC/half_2/".format(ty)
                )
                dir_key["dir_noise_{}_1".format(ty[0:3])] = (
                    self.save_dir + "noise/{}/half_1/".format(ty)
                )
                dir_key["dir_noise_{}_2".format(ty[0:3])] = (
                    self.save_dir + "noise/{}/half_2/".format(ty)
                )
                dir_key["dir_cmb_obs_{}_1".format(ty[0:3])] = (
                    self.save_dir + "observed/{}/cmb/half_1/".format(ty)
                )
                dir_key["dir_cmb_ilc_{}_1".format(ty[0:3])] = (
                    self.save_dir + "observed/{}/cmb_ilcnoise/half_1/".format(ty)
                )
                dir_key["dir_total_obs_{}_1".format(ty[0:3])] = (
                    self.save_dir + "observed/{}/total/half_1/".format(ty)
                )
                dir_key["dir_cmb_obs_{}_2".format(ty[0:3])] = (
                    self.save_dir + "observed/{}/cmb/half_2/".format(ty)
                )
                dir_key["dir_cmb_ilc_{}_2".format(ty[0:3])] = (
                    self.save_dir + "observed/{}/cmb_ilcnoise/half_2/".format(ty)
                )
                dir_key["dir_total_obs_{}_2".format(ty[0:3])] = (
                    self.save_dir + "observed/{}/total/half_2/".format(ty)
                )
                dir_key["dir_cmb_obs_flat_{}_1".format(ty[0:3])] = (
                    self.save_dir + "observed_flat_map/{}/cmb/half_1/".format(ty)
                )
                dir_key["dir_cmb_ilc_flat_{}_1".format(ty[0:3])] = (
                    self.save_dir
                    + "observed_flat_map/{}/cmb_ilcnoise/half_1/".format(ty)
                )
                dir_key["dir_total_obs_flat_{}_1".format(ty[0:3])] = (
                    self.save_dir + "observed_flat_map/{}/total/half_1/".format(ty)
                )
                dir_key["dir_cmb_obs_flat_{}_2".format(ty[0:3])] = (
                    self.save_dir + "observed_flat_map/{}/cmb/half_2/".format(ty)
                )
                dir_key["dir_cmb_ilc_flat_{}_2".format(ty[0:3])] = (
                    self.save_dir
                    + "observed_flat_map/{}/cmb_ilcnoise/half_2/".format(ty)
                )
                dir_key["dir_total_obs_flat_{}_2".format(ty[0:3])] = (
                    self.save_dir + "observed_flat_map/{}/total/half_2/".format(ty)
                )
                dir_key["dir_cmb_obs_flat_block_{}_1".format(ty[0:3])] = (
                    self.save_dir + "observed_flat_block_map/{}/cmb/half_1/".format(ty)
                )
                dir_key["dir_cmb_ilc_flat_block_{}_1".format(ty[0:3])] = (
                    self.save_dir
                    + "observed_flat_block_map/{}/cmb_ilcnoise/half_1/".format(ty)
                )
                dir_key["dir_total_obs_flat_block_{}_1".format(ty[0:3])] = (
                    self.save_dir
                    + "observed_flat_block_map/{}/total/half_1/".format(ty)
                )
                dir_key["dir_cmb_obs_flat_block_{}_2".format(ty[0:3])] = (
                    self.save_dir + "observed_flat_block_map/{}/cmb/half_2/".format(ty)
                )
                dir_key["dir_cmb_ilc_flat_block_{}_2".format(ty[0:3])] = (
                    self.save_dir
                    + "observed_flat_block_map/{}/cmb_ilcnoise/half_2/".format(ty)
                )
                dir_key["dir_total_obs_flat_block_{}_2".format(ty[0:3])] = (
                    self.save_dir
                    + "observed_flat_block_map/{}/total/half_2/".format(ty)
                )
    ...
    def __get_Data(self):
        return skm.Get_data(
            ...
            config_random=self.Config_random,
            using_beam=self.using_beam,
            beam=self.beams,
        )
    ...
    def simulator_sky_map(self, N_sky_map):
        ...
        for n in range(N_sky_map):
            ...
            np.save(
                self.dir_total + "total" + str(n) + ".npy", total.astype(np.float32)
            )
            pbar.update(1)
    ...
    def simulator_noise_map(self, N_noise_map, dataset_type="training_set"):
        ...
        if dataset_type == "training_set":
            ...
            print(
                'An error occurred: Please set the dataset_type correctly. dataset_type = "training_set" or "validation_set" or "testing_set"'
            )
        ...
        for nn in range(N_noise_map):
            if dataset_type == "training_set":
                ...
                np.save(
                    self.dir_noise_tra + "noise" + str(nn) + ".npy",
                ...
                if self.is_half_split_map:
                    ...
                    np.save(
                        self.dir_noise_val_1 + "noise" + str(nn) + ".npy",
                        noise_1.astype(np.float32),
                    )
                    noise_2 = self.__get_noise()
                    np.save(
                        self.dir_noise_val_2 + "noise" + str(nn) + ".npy",
                        noise_2.astype(np.float32),
                    )
                else:
                    noise_1 = self.__get_noise()
                    np.save(
                        self.dir_noise_val + "noise" + str(nn) + ".npy",
                        noise_1.astype(np.float32),
                    )
            elif dataset_type == "testing_set":
                if self.is_half_split_map:
                    noise_1 = self.__get_noise()
                    np.save(
                        self.dir_noise_tes_1 + "noise" + str(nn) + ".npy",
                        noise_1.astype(np.float32),
                    )
                    noise_2 = self.__get_noise()
                    np.save(
                        self.dir_noise_tes_2 + "noise" + str(nn) + ".npy",
                        noise_2.astype(np.float32),
                    )
                else:
                    noise_1 = self.__get_noise()
                    np.save(
                        self.dir_noise_tes + "noise" + str(nn) + ".npy",
                        noise_1.astype(np.float32),
                    )
            pbar.update(1)
    ...
    def get_observed_map(
        self, index_sky_map, index_noise_map, dataset_type="training_set"
    ):
        """
        ...
        :return:
        """
        index_sky_map, index_noise_map = self.__map_index(
            index_sky_map, index_noise_map
        )
        ...
        for idx, _ in enumerate(zip(index_sky_map, index_noise_map)):
            ...
            if dataset_type == "training_set":
                ...
                np.save(
                    self.dir_cmb_obs_tra + "cmb" + str(idx) + ".npy",
                    (cmb + noise).astype(np.float32),
                )
                np.save(
                    self.dir_total_obs_tra + "total" + str(idx) + ".npy",
                    (total + noise).astype(np.float32),
                )
                np.save(
                    self.dir_noise_tra_reshape + "noise" + str(idx) + ".npy",
                    (noise).astype(np.float32),
                )
                ...
                if self.is_half_split_map:
                    ...
                    np.save(
                        self.dir_cmb_obs_val_1 + "cmb" + str(idx) + ".npy",
                        (cmb + noise1).astype(np.float32),
                    )
                    np.save(
                        self.dir_cmb_obs_val_2 + "cmb" + str(idx) + ".npy",
                        (cmb + noise2).astype(np.float32),
                    )
                    np.save(
                        self.dir_total_obs_val_1 + "total" + str(idx) + ".npy",
                        (total + noise1).astype(np.float32),
                    )
                    np.save(
                        self.dir_total_obs_val_2 + "total" + str(idx) + ".npy",
                        (total + noise2).astype(np.float32),
                    )
                else:
                    noise1 = np.load(self.dir_noise_val + "noise" + str(n) + ".npy")
                    np.save(
                        self.dir_cmb_obs_val + "cmb" + str(idx) + ".npy",
                        (cmb + noise1).astype(np.float32),
                    )
                    np.save(
                        self.dir_total_obs_val + "total" + str(idx) + ".npy",
                        (total + noise1).astype(np.float32),
                    )
                ...
                if self.is_half_split_map:
                    ...
                    np.save(
                        self.dir_cmb_obs_tes_1 + "cmb" + str(idx) + ".npy",
                        (cmb + noise1).astype(np.float32),
                    )
                    np.save(
                        self.dir_cmb_obs_tes_2 + "cmb" + str(idx) + ".npy",
                        (cmb + noise2).astype(np.float32),
                    )
                    np.save(
                        self.dir_total_obs_tes_1 + "total" + str(idx) + ".npy",
                        (total + noise1).astype(np.float32),
                    )
                    np.save(
                        self.dir_total_obs_tes_2 + "total" + str(idx) + ".npy",
                        (total + noise2).astype(np.float32),
                    )
                else:
                    noise1 = np.load(self.dir_noise_tes + "noise" + str(n) + ".npy")
                    np.save(
                        self.dir_cmb_obs_tes + "cmb" + str(idx) + ".npy",
                        (cmb + noise1).astype(np.float32),
                    )
                    np.save(
                        self.dir_total_obs_tes + "total" + str(idx) + ".npy",
                        (total + noise1).astype(np.float32),
                    )
            pbar.update(1)
    ...
    def run_ilc(self, N_samp, out_freq, mask=None, dataset_type="training_set"):
        ...
        for k, v in zip(bands, beam):
            beam_profile["beam_" + str(k)] = hp.gauss_beam(
                v * np.pi / 10800.0, lmax=self.nside * 3, pol=False
            )
        ...
        for idx in N_samp:
            if dataset_type == "training_set":
                ...
                np.save(
                    self.dir_noise_ILC_tra + "ilcnoise{}.npy".format(idx),
                    _maps.astype(np.float32),
                )
            elif dataset_type == "validation_set":
                if self.is_half_split_map:
                    data_qu_1 = np.load(
                    ...
                    noise_1 = np.load(
                        self.dir_noise_val_1 + "noise" + str(idx) + ".npy"
                    )
                    noise_2 = np.load(
                        self.dir_noise_val_2 + "noise" + str(idx) + ".npy"
                    )
                    ...
                    np.save(
                        self.dir_noise_ILC_val_1 + "ilcnoise{}.npy".format(idx),
                        _maps_1.astype(np.float32),
                    )
                    np.save(
                        self.dir_noise_ILC_val_2 + "ilcnoise{}.npy".format(idx),
                        _maps_2.astype(np.float32),
                    )
                else:
                    data_qu = np.load(
                        self.dir_total_obs_val + "total" + str(idx) + ".npy"
                    ...
                    np.save(
                        self.dir_noise_ILC_val + "ilcnoise{}.npy".format(idx),
                        _maps.astype(np.float32),
                    )
                ...
                if self.is_half_split_map:
                    ...
                    noise_1 = np.load(
                        self.dir_noise_tes_1 + "noise" + str(idx) + ".npy"
                    )
                    noise_2 = np.load(
                        self.dir_noise_tes_2 + "noise" + str(idx) + ".npy"
                    )
                    resmap_1 = __ilc(data_qu_1, noise_1)
                    resmap_2 = __ilc(data_qu_2, noise_2)
                    if not self.is_polarization_data:
                    ...
                    np.save(
                        self.dir_noise_ILC_tes_1 + "ilcnoise{}.npy".format(idx),
                        _maps_1.astype(np.float32),
                    )
                    np.save(
                        self.dir_noise_ILC_tes_2 + "ilcnoise{}.npy".format(idx),
                        _maps_2.astype(np.float32),
                    )
                else:
                    data_qu = np.load(
                    ...
                    np.save(
                        self.dir_noise_ILC_tes + "ilcnoise{}.npy".format(idx),
                        _maps.astype(np.float32),
                    )

            pbar.update(1)

    def mult_process_get_ilc_noise(
        self, N_sample, N_mult=10, out_freq=220, mask=None, dataset_type="training_set"
    ):
        """
    ...
    def get_data_CMB_ilcnoise(
        ...
        for idx, _ in enumerate(zip(index_sky_map, index_ilcnoise_map)):
            ...
            if dataset_type == "training_set":
                ...
                np.save(
                    self.dir_cmb_ilc_tra + "cmb" + str(idx) + ".npy",
                    (cmb + noise).astype(np.float32),
                )
            elif dataset_type == "validation_set":
                if self.is_half_split_map:
                    noise_1 = np.load(
                        self.dir_noise_ILC_val_1 + "ilcnoise{}.npy".format(n)
                    )
                    noise_2 = np.load(
                        self.dir_noise_ILC_val_2 + "ilcnoise{}.npy".format(n)
                    )
                    self._creat_file(self.dir_cmb_ilc_val_1)
                    self._creat_file(self.dir_cmb_ilc_val_2)
                    np.save(
                        self.dir_cmb_ilc_val_1 + "cmb" + str(idx) + ".npy",
                        (cmb + noise_1).astype(np.float32),
                    )
                    np.save(
                        self.dir_cmb_ilc_val_2 + "cmb" + str(idx) + ".npy",
                        (cmb + noise_2).astype(np.float32),
                    )
                else:
                    noise = np.load(self.dir_noise_ILC_val + "ilcnoise{}.npy".format(n))
                    self._creat_file(self.dir_cmb_ilc_val)
                    np.save(
                        self.dir_cmb_ilc_val + "cmb" + str(idx) + ".npy",
                        (cmb + noise).astype(np.float32),
                    )
            elif dataset_type == "testing_set":
                if self.is_half_split_map:
                    noise_1 = np.load(
                        self.dir_noise_ILC_tes_1 + "ilcnoise{}.npy".format(n)
                    )
                    noise_2 = np.load(
                        self.dir_noise_ILC_tes_2 + "ilcnoise{}.npy".format(n)
                    )
                    self._creat_file(self.dir_cmb_ilc_tes_1)
                    self._creat_file(self.dir_cmb_ilc_tes_2)
                    np.save(
                        self.dir_cmb_ilc_tes_1 + "cmb" + str(idx) + ".npy",
                        (cmb + noise_1).astype(np.float32),
                    )
                    np.save(
                        self.dir_cmb_ilc_tes_2 + "cmb" + str(idx) + ".npy",
                        (cmb + noise_2).astype(np.float32),
                    )
                else:
                    ...
                    np.save(
                        self.dir_cmb_ilc_tes + "cmb" + str(idx) + ".npy",
                        (cmb + noise).astype(np.float32),
                    )
            pbar.update(1)
...
class Data_preprocessing(Simulator_data):
    ...
    def flat_map_from_sphere_fullsky(self, sphere_map):
        if len(sphere_map.shape) == 3:
            ...
            for j in range(comp_p):
                map_new[:, j, :, :] = sp.sphere2piecePlane_squa_mult(
                    sphere_map=sphere_map[:, j, :], nside=self.nside
                )
        else:
            map_new = sp.sphere2piecePlane_squa_mult(
                sphere_map=sphere_map, nside=self.nside
            )
        return map_new

    def flat_map_from_sphere_single(self, sphere_map, block_index=0):
        if len(sphere_map.shape) == 3:
            freqs, comp_p, N_pix = sphere_map.shape
            map_new = np.zeros((freqs, comp_p, self.nside, self.nside))
            for j in range(comp_p):
                map_new[:, j, :, :] = sp.sphere2piecePlane_single(
                    sphere_map=sphere_map[:, j, :],
                    nside=self.nside,
                    block_index=block_index,
                )
        else:
            map_new = sp.sphere2piecePlane_single(
                sphere_map=sphere_map, nside=self.nside, block_index=block_index
            )
        return map_new
    ...
    def flat_map_from_sphere_fullsky_padding(self, sphere_map):
        if len(sphere_map.shape) == 3:
            freqs, comp_p, N_pix = sphere_map.shape
            map_new = np.ones(
                (freqs, comp_p, 5 * self.nside + 128 * 2, 5 * self.nside + 128 * 2)
            )
            for j in range(comp_p):
                map_new[:, j, :, :] = sp.sphere2piecePlane_squa_pad_mult(
                    sphere_map=sphere_map[:, j, :], nside=self.nside
                )
        else:
            map_new = sp.sphere2piecePlane_squa_pad_mult(
                sphere_map=sphere_map, nside=self.nside
            )
    ...
    def flat_map_from_sphere_block_map(self, sphere_map):
        if len(sphere_map.shape) == 3:
            ...
            for j in range(comp_p):
                map_new[:, j, :, :] = sp.sphere2piecePlane_mult(
                    sphere_map=sphere_map[:, j, :], nside=self.nside
                )
                map_new_block[:, j, :, :] = sp.piecePlanes2blocks_mult(
                    piece_maps=map_new[:, j, :, :], nside=self.nside, block_n=self.block
                )
        else:
            map_new = sp.sphere2piecePlane_mult(sphere_map=sphere_map, nside=self.nside)
            map_new_block = sp.piecePlanes2blocks_mult(
                piece_maps=map_new[:, :], nside=self.nside, block_n=self.block
            )
        ...
        for idx in Nsample:
            if self.dataset_type == "training_set":
                ...
                if self.padding:
                    ...
                    total_obs_flat = self.flat_map_from_sphere_fullsky_padding(
                        total_obs
                    )
                else:
                ...
                if self.using_ilc_cmbmap:
                    ...
                    np.save(
                        self.dir_cmb_ilc_flat_tra + "cmb" + str(idx) + ".npy",
                        (cmb_obs_flat).astype(np.float32),
                    )
                else:
                    self._creat_file(self.dir_cmb_obs_flat_tra)
                    np.save(
                        self.dir_cmb_obs_flat_tra + "cmb" + str(idx) + ".npy",
                        (cmb_obs_flat).astype(np.float32),
                    )
                self._creat_file(self.dir_total_obs_flat_tra)
                np.save(
                    self.dir_total_obs_flat_tra + "total" + str(idx) + ".npy",
                ...
                if self.is_half_split_map:
                    ...
                    if self.padding:
                        cmb_obs_flat_1 = self.flat_map_from_sphere_fullsky_padding(
                            cmb_obs_1
                        )
                        cmb_obs_flat_2 = self.flat_map_from_sphere_fullsky_padding(
                            cmb_obs_2
                        )
                        total_obs_flat_1 = self.flat_map_from_sphere_fullsky_padding(
                            total_obs_1
                        )
                        total_obs_flat_2 = self.flat_map_from_sphere_fullsky_padding(
                            total_obs_2
                        )
                    else:
                        cmb_obs_flat_1 = self.flat_map_from_sphere_fullsky(cmb_obs_1)
                        cmb_obs_flat_2 = self.flat_map_from_sphere_fullsky(cmb_obs_2)
                        total_obs_flat_1 = self.flat_map_from_sphere_fullsky(
                            total_obs_1
                        )
                        total_obs_flat_2 = self.flat_map_from_sphere_fullsky(
                            total_obs_2
                        )
                    if self.using_ilc_cmbmap:
                        ...
                        np.save(
                            self.dir_cmb_ilc_flat_val_1 + "cmb" + str(idx) + ".npy",
                            (cmb_obs_flat_1).astype(np.float32),
                        )
                        np.save(
                            self.dir_cmb_ilc_flat_val_2 + "cmb" + str(idx) + ".npy",
                        ...
                        np.save(
                            self.dir_cmb_obs_flat_val_1 + "cmb" + str(idx) + ".npy",
                            (cmb_obs_flat_1).astype(np.float32),
                        )
                        np.save(
                            self.dir_cmb_obs_flat_val_2 + "cmb" + str(idx) + ".npy",
                            (cmb_obs_flat_2).astype(np.float32),
                        )
                    self._creat_file(self.dir_total_obs_flat_val_1)
                    self._creat_file(self.dir_total_obs_flat_val_2)
                    np.save(
                        self.dir_total_obs_flat_val_1 + "total" + str(idx) + ".npy",
                        (total_obs_flat_1).astype(np.float32),
                    )
                    np.save(
                        self.dir_total_obs_flat_val_2 + "total" + str(idx) + ".npy",
                    ...
                    if self.padding:
                        cmb_obs_flat = self.flat_map_from_sphere_fullsky_padding(
                            cmb_obs
                        )
                        total_obs_flat = self.flat_map_from_sphere_fullsky_padding(
                            total_obs
                    ...
                    if self.using_ilc_cmbmap:
                        ...
                        np.save(
                            self.dir_cmb_ilc_flat_val + "cmb" + str(idx) + ".npy",
                            (cmb_obs_flat).astype(np.float32),
                        )
                    else:
                        self._creat_file(self.dir_cmb_obs_flat_val)
                        np.save(
                            self.dir_cmb_obs_flat_val + "cmb" + str(idx) + ".npy",
                            (cmb_obs_flat).astype(np.float32),
                        )
                    self._creat_file(self.dir_total_obs_flat_val)
                    np.save(
                        self.dir_total_obs_flat_val + "total" + str(idx) + ".npy",
                        (total_obs_flat).astype(np.float32),
                    )
                ...
                if self.is_half_split_map:
                    ...
                    if self.using_ilc_cmbmap:
                        ...
                        np.save(
                            self.dir_cmb_ilc_flat_tes_1 + "cmb" + str(idx) + ".npy",
                            (cmb_obs_flat_1).astype(np.float32),
                        )
                        np.save(
                            self.dir_cmb_ilc_flat_tes_2 + "cmb" + str(idx) + ".npy",
                            (cmb_obs_flat_2).astype(np.float32),
                        )
                    else:
                        ...
                        np.save(
                            self.dir_cmb_obs_flat_tes_1 + "cmb" + str(idx) + ".npy",
                            (cmb_obs_flat_1).astype(np.float32),
                        )
                        np.save(
                            self.dir_cmb_obs_flat_tes_2 + "cmb" + str(idx) + ".npy",
                            (cmb_obs_flat_2).astype(np.float32),
                        )
                    self._creat_file(self.dir_total_obs_flat_tes_1)
                    self._creat_file(self.dir_total_obs_flat_tes_2)
                    np.save(
                        self.dir_total_obs_flat_tes_1 + "total" + str(idx) + ".npy",
                        (total_obs_flat_1).astype(np.float32),
                    )
                    np.save(
                        self.dir_total_obs_flat_tes_2 + "total" + str(idx) + ".npy",
                        (total_obs_flat_2).astype(np.float32),
                    )
                else:
                    if self.using_ilc_cmbmap:
                    ...
                    if self.using_ilc_cmbmap:
                        ...
                        np.save(
                            self.dir_cmb_ilc_flat_tes + "cmb" + str(idx) + ".npy",
                            (cmb_obs_flat).astype(np.float32),
                        )
                    else:
                        self._creat_file(self.dir_cmb_obs_flat_tes)
                        np.save(
                            self.dir_cmb_obs_flat_tes + "cmb" + str(idx) + ".npy",
                            (cmb_obs_flat).astype(np.float32),
                        )
                    self._creat_file(self.dir_total_obs_flat_tes)
                    np.save(
                        self.dir_total_obs_flat_tes + "total" + str(idx) + ".npy",
                        (total_obs_flat).astype(np.float32),
                    )
            else:
                print(
        ...
        for idx in Nsample:
            if self.dataset_type == "training_set":
                ...
                if self.using_ilc_cmbmap:
                    ...
                    np.save(
                        self.dir_cmb_ilc_flat_block_tra + "cmb" + str(idx) + ".npy",
                        (cmb_obs_flat).astype(np.float32),
                    )
                else:
                    self._creat_file(self.dir_cmb_obs_flat_block_tra)
                    np.save(
                        self.dir_cmb_obs_flat_block_tra + "cmb" + str(idx) + ".npy",
                        (cmb_obs_flat).astype(np.float32),
                    )
                self._creat_file(self.dir_total_obs_flat_block_tra)
                np.save(
                    self.dir_total_obs_flat_block_tra + "total" + str(idx) + ".npy",
                    (total_obs_flat).astype(np.float32),
                )
                ...
                if self.is_half_split_map:
                    ...
                    if self.using_ilc_cmbmap:
                        ...
                        np.save(
                            self.dir_cmb_ilc_flat_block_val_1
                            + "cmb"
                            + str(idx)
                            + ".npy",
                            (cmb_obs_flat_1).astype(np.float32),
                        ...
                        np.save(
                            self.dir_cmb_ilc_flat_block_val_2
                            + "cmb"
                            + str(idx)
                            + ".npy",
                            (cmb_obs_flat_2).astype(np.float32),
                        ...
                        np.save(
                            self.dir_cmb_obs_flat_block_val_1
                            + "cmb"
                            + str(idx)
                            + ".npy",
                            (cmb_obs_flat_1).astype(np.float32),
                        ...
                        np.save(
                            self.dir_cmb_obs_flat_block_val_2
                            + "cmb"
                            + str(idx)
                            + ".npy",
                            (cmb_obs_flat_2).astype(np.float32),
                    ...
                    np.save(
                        self.dir_total_obs_flat_block_val_1
                        + "total"
                        + str(idx)
                        + ".npy",
                        (total_obs_flat_1).astype(np.float32),
                    ...
                    np.save(
                        self.dir_total_obs_flat_block_val_2
                        + "total"
                        + str(idx)
                        + ".npy",
                        (total_obs_flat_2).astype(np.float32),
                    ...
                    if self.using_ilc_cmbmap:
                        ...
                        np.save(
                            self.dir_cmb_ilc_flat_block_val + "cmb" + str(idx) + ".npy",
                            (cmb_obs_flat).astype(np.float32),
                        )
                    else:
                        self._creat_file(self.dir_cmb_obs_flat_block_val)
                        np.save(
                            self.dir_cmb_obs_flat_block_val + "cmb" + str(idx) + ".npy",
                            (cmb_obs_flat).astype(np.float32),
                        )
                    self._creat_file(self.dir_total_obs_flat_block_val)
                    np.save(
                        self.dir_total_obs_flat_block_val + "total" + str(idx) + ".npy",
                        (total_obs_flat).astype(np.float32),
                    )
                ...
                if self.is_half_split_map:
                    ...
                    if self.using_ilc_cmbmap:
                        ...
                        np.save(
                            self.dir_cmb_ilc_flat_block_tes_1
                            + "cmb"
                            + str(idx)
                            + ".npy",
                            (cmb_obs_flat_1).astype(np.float32),
                        ...
                        np.save(
                            self.dir_cmb_ilc_flat_block_tes_2
                            + "cmb"
                            + str(idx)
                            + ".npy",
                            (cmb_obs_flat_2).astype(np.float32),
                        ...
                        np.save(
                            self.dir_cmb_obs_flat_block_tes_1
                            + "cmb"
                            + str(idx)
                            + ".npy",
                            (cmb_obs_flat_1).astype(np.float32),
                        ...
                        np.save(
                            self.dir_cmb_obs_flat_block_tes_2
                            + "cmb"
                            + str(idx)
                            + ".npy",
                            (cmb_obs_flat_2).astype(np.float32),
                    ...
                    np.save(
                        self.dir_total_obs_flat_block_tes_1
                        + "total"
                        + str(idx)
                        + ".npy",
                        (total_obs_flat_1).astype(np.float32),
                    ...
                    np.save(
                        self.dir_total_obs_flat_block_tes_2
                        + "total"
                        + str(idx)
                        + ".npy",
                        (total_obs_flat_2).astype(np.float32),
                    ...
                    if self.using_ilc_cmbmap:
                        ...
                        np.save(
                            self.dir_cmb_ilc_flat_block_tes + "cmb" + str(idx) + ".npy",
                            (cmb_obs_flat).astype(np.float32),
                        )
                    else:
                        self._creat_file(self.dir_cmb_obs_flat_block_tes)
                        np.save(
                            self.dir_cmb_obs_flat_block_tes + "cmb" + str(idx) + ".npy",
                            (cmb_obs_flat).astype(np.float32),
                        )
                    self._creat_file(self.dir_total_obs_flat_block_tes)
                    np.save(
                        self.dir_total_obs_flat_block_tes + "total" + str(idx) + ".npy",
                        (total_obs_flat).astype(np.float32),
                    )
            else:
                print(
    ...
    def mult_process_get_flatmap_from_spheremap(
        self, N_mult, N_sample, dataset_type="training_set"
    ):
        """
        ...
        if self.full_sky_map:
            processes = [
                multiprocessing.Process(
                    target=self.get_flat_map_from_sphere_fullsky,
                    kwargs={"Nsample": S_N[i]},
                )
                for i in range(len(S_N))
            ]
        else:
            processes = [
                multiprocessing.Process(
                    target=self.get_flat_map_from_sphere_blocksky,
                    kwargs={"Nsample": S_N[i]},
                )
                for i in range(len(S_N))
            ]
    ...
    def sphere_map_from_fla_map_fullsky(self, piece_map):
        if len(piece_map) == 4:
            ...
            for j in range(comp_p):
                map_new[:, j, :, :] = sp.piecePlane_squa2sphere_mult(
                    piece_map[:, j, :], nside=self.nside
                )
        else:
    ...
    def sphere_map_from_flat_map_fullsky_padding(self, piece_map):
        if len(piece_map) == 4:
            ...
            map_new = np.zeros((freqs, comp_p, 12 * self.nside**2))
            for j in range(comp_p):
                map_new[:, j, :, :] = sp.piecePlane_squa2sphere_pad_mult(
                    piece_map[:, j, :], nside=self.nside
                )
        else:
    ...
    def sphere_from_flat_block_map(self, block_map, block="block_0"):
        if len(block_map) == 4:
            ...
            map_new = np.zeros((freqs, comp_p, 12 * self.nside**2))
            for j in range(comp_p):
                map_new[:, j, :] = sp.blockPlane2sphere_mult(
                    block_map[:, j, :], nside=self.nside, block_n=block
                )
        else:
            map_new = sp.blockPlane2sphere_mult(
                block_map, nside=self.nside, block_n=block
            )
