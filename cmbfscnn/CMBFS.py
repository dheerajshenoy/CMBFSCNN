...
class CMBFSCNN(object):
    ...
    def _get_config(self):
        ...
        self.data_index = [
            np.arange(self.N_sky_maps[0]),
            np.arange(self.N_sky_maps[1], np.sum(self.N_sky_maps[0:2])),
    ...
    def data_simulation(self):
        Sim_data = gd.Simulator_data(
            freqs=self.freqs,
            beams=self.beams,
            noise_level=self.sens,
            is_half_split_map=self.is_half_split_map,
            using_ilc_cmbmap=self.using_ilc_cmbmap,
            Nside=self.nside,
            config_random=self.data_config_random,
        ...
        for k, data_type in enumerate(self.dataset_type):
            Sim_data.get_observed_map(
                index_sky_map=self.data_index[k],
                index_noise_map=self.noise_index[k],
    ...
    def cal_ilc_noise(self):
        ...
        if self.using_ilc_cmbmap:
            for k, data_type in enumerate(self.dataset_type):
                Sim_data.mult_process_get_ilc_noise(
                    N_sample=self.N_sky_maps[k],
                    N_mult=self.ILC_N_threads,
                    out_freq=self.output_freq,
                    mask=self.ilc_mask,
                    dataset_type=data_type,
                )
            for k, data_type in enumerate(self.dataset_type):
                if data_type == "training_set":
                    Sim_data.get_data_CMB_ilcnoise(
                        index_sky_map=self.data_index[k],
    ...
    def data_preprocessing(self):
        ...
        for k, data_type in enumerate(self.dataset_type):
            Data_prep.mult_process_get_flatmap_from_spheremap(
                N_mult=self.N_threads_preprocessing,
                N_sample=self.N_sky_maps[k],
                dataset_type=data_type,
            )

    def _training_cnn(self, component="Q", using_loss_fft=True, repeat_n=3):
        FSM_Qmap = Cmo.Foreground_subtraction_model(
            data_dir=self.save_data_dir,
            freqs=self.freqs,
        ...
        FSM_Qmap.net_train(
            ...
            num_train=self.N_sky_maps[0],
            num_validation=self.N_sky_maps[1],
            device_ids=self.device_ids,
            iteration=self.iteration,
            CNN_model=self.CNN_model,
            using_loss_fft=using_loss_fft,
            repeat_n=repeat_n,
        )
    ...
    def get_predicted_maps(self):
        RA = Cmo.Result_analysis(
            data_dir=self.save_data_dir,
            full_sky_map=self.is_fullsky,
            map_block=self.block_n,
            padding=self.padding,
            using_ilc_cmbmap=self.using_ilc_cmbmap,
            nside=self.nside,
            is_half_split_map=self.is_half_split_map,
    ...
    def calculate_power_spectra(self, nlb=5, Dl=True):
        CPS = Cmo.Calculate_power_spectra(
            result_dir=self.save_result_dir,
            is_half_split_map=self.is_half_split_map,
            beams_arcmin=self.beams,
            output_beam_arcmin=self.output_beam,
            N_sample=self.N_sky_maps[2],
            nside=self.nside,
        ...
        if "T" in self.component:
            CPS.get_true_CMB_PS(
                nlb=nlb, Dl=Dl, aposize=None, EB_power=False
            )  # get true CMB TT and TT power spectra
            CPS.cal_cmb_T_PS(
                nlb=nlb, Dl=Dl, aposize=None, EB_power=False
            )  # get output CMB TT and TT power spectra
            if self.is_half_split_map:
                CPS.cal_cmb_T_cross_PS(
                    nlb=nlb, Dl=Dl, aposize=None, EB_power=False
                )  # get output CMB TT and TT power spectra after denoising step

        if "Q" in self.component and "U" in self.component:
            CPS.get_true_CMB_PS(
                nlb=nlb, Dl=Dl, aposize=None, EB_power=False
            )  # get true CMB QQ and UU power spectra
            CPS.get_true_CMB_PS(
                nlb=nlb, Dl=Dl, aposize=None, EB_power=True
            )  # get true CMB EE and BB power spectra
            CPS.cal_cmb_Q_or_U_PS(
                nlb=nlb, Dl=Dl, aposize=None, EB_power=False
            )  # get output CMB QQ and UU power spectra
            CPS.cal_cmb_Q_or_U_cross_PS(
                nlb=nlb, Dl=Dl, aposize=None, EB_power=False
            )  # get output CMB QQ and UU power spectra after denoising step
            CPS.cal_cmb_E_B_PS(
                nlb=nlb, Dl=Dl, aposize=None, EB_power=True
            )  # get output CMB EE and BB power spectra
            CPS.cal_cmb_E_B_cross_PS(
                nlb=nlb, Dl=Dl, aposize=None, EB_power=True
            )  # get output CMB EE and BB power spectra after denoising step
